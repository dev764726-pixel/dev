    ...
    def _calculate_indicators(self, symbol, token):
        try:
            ...
            if not ret or not isinstance(ret, list) or len(ret) == 0:
                logging.warning(f"{symbol}: Could not fetch valid 15-min data from API.")
                return None
            df = pd.DataFrame(ret)
            df['time'] = pd.to_datetime(df['time'], format='%d-%m-%Y %H:%M:%S')
            df.set_index('time', inplace=True)

            # Robustly convert columns to numeric, coercing errors to NaN
            for col in ['inth', 'intl', 'intc', 'into', 'intv']:
                df[col] = pd.to_numeric(df[col], errors='coerce')
            df.dropna(inplace=True) # Drop rows with any invalid data

            df = df.iloc[::-1] # Reverse dataframe to have oldest data first
            ...
            if not daily_ret or not isinstance(daily_ret, list) or len(daily_ret) == 0:
                logging.warning(f"{symbol}: Could not fetch valid daily data from API.")
                return None
            daily_df = pd.DataFrame(daily_ret)
            daily_df['v'] = pd.to_numeric(daily_df['v'], errors='coerce')
            daily_df['c'] = pd.to_numeric(daily_df['c'], errors='coerce')
            daily_df.dropna(inplace=True)
            if daily_df.empty:
                logging.warning(f"{symbol}: Daily data was invalid after cleaning.")
                return None
            return {
