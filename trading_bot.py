...
class FlattradeTrader:
    ...
    def _calculate_indicators(self, symbol, token):
        try:
            ...
            # --- 15-Minute Data Fetching with Error Handling ---
            try:
                ...
                logging.warning(f"{symbol}: API call for 15-min data failed. Error: {e}")
                return None

            if not ret or not isinstance(ret, list) or len(ret) == 0:
                logging.warning(f"{symbol}: API returned invalid 15-min data.")
                return None
